...
void main_loop(){	
	if((clock_time()-last_delay) > 5000*CLOCK_SYS_CLOCK_1MS){//main loop delay
		...
		if(meas_count >= settings.measure_interval){
			...
			temp += 0;//settings.temp_offset;
			humi += 0;//settings.humi_offset;
			meas_count=0;
		...
		if(settings.temp_C_or_F){
			...
			show_temp_symbol(1);
			if (settings.forced_temp != 0)
			{
				int16_t tt = (settings.forced_temp * 10) + settings.forced_temp_dot;
				show_big_number(tt,1);
			}
			else
			{
